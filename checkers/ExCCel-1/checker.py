...
def check_sla_3(host):
    ...
    for _ in range(random.randrange(4, 5)):
...
def put_flag(host, flag):
    ...
    try:
        w = ch.save_worksheet(w['id'], [{'x': cell_x, 'y': cell_y, 'content': flag.encode().hex()}])
    except RequestException:
        quit(Status.DOWN, 'Could not save worksheet', nice_logs(logs))

    # Post flag id to game server
    try:
        post_flag_id('ExCCel-1', team_id, {'worksheet_id' : w['id']})
    except Exception:
...
def get_flag(host, flag):
    ...
    for cell in w['cells']:
        if (cell['x'] == flag_data['cell_x'] and cell['y'] == flag_data['cell_y']
                and cell['content'] == flag.encode().hex()):
            break
    else:
        quit(Status.DOWN, 'Could not find flag in worksheet', nice_logs(logs, f'Cells: {w["cells"]}'))
